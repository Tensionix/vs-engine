...
>>"%MENU_FILE%" echo [03] Polaroid                              ^| polaroid        ^| 1970s SX-70: candy-bloom, тёплая кремовость, виньетка
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo === Сервис ========================================================
>>"%MENU_FILE%" echo [04] Список энкодеров                      ^| list_encoders   ^| H.264 / H.265 / ProRes / DNxHR
>>"%MENU_FILE%" echo [05] Доктор                                ^| doctor          ^| диагностика стека
>>"%MENU_FILE%" echo [06] Открыть папку input                   ^| open_input      ^| Explorer
>>"%MENU_FILE%" echo [07] Открыть папку output                  ^| open_output     ^| Explorer
>>"%MENU_FILE%" echo [08] Открыть папку logs                    ^| open_logs       ^| JSON отчёты
...
if /I "%RAW%"=="polaroid"       goto POLAROID
...
echo [3] Polaroid              (SX-70 vibe, candy-bloom, виньетка)
echo.
echo === Сервис ===
echo [4] Список энкодеров
echo [5] Доктор
...
if errorlevel 8 goto OPEN_LOGS
if errorlevel 7 goto OPEN_OUTPUT
if errorlevel 6 goto OPEN_INPUT
if errorlevel 5 goto DOCTOR
if errorlevel 4 goto LIST_ENCODERS
if errorlevel 3 goto POLAROID
...
:POLAROID
call :ASK_INPUT_FILE
call :ASK_OUTPUT_FILE polaroid
call :ASK_INTENSITY
call :ASK_VIGNETTE
call :ASK_ENCODER
call :RUN_PRESET polaroid --intensity %INTENSITY% --vignette %VIGNETTE%
goto MAIN
...
set "VIGNETTE="
echo.
echo Edge виньетка (затемнение углов; 0 = нет, 1 = углы в чёрный):
echo   1 - 0.30  (тонко)
echo   2 - 0.55  (Polaroid по умолчанию)
echo   3 - 0.80  (сильно)
set /p VIGNETTE=Выбор [2] :
if not defined VIGNETTE set "VIGNETTE=0.55"
if /I "%VIGNETTE%"=="1" set "VIGNETTE=0.30"
if /I "%VIGNETTE%"=="2" set "VIGNETTE=0.55"
if /I "%VIGNETTE%"=="3" set "VIGNETTE=0.80"
goto :eof
