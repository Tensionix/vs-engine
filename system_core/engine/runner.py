...
def run_pipeline(
    *,
    ...
    # Drain both stderr pipes concurrently. VapourSynth R75/plugin autoload can
    # emit enough API-deprecation warnings to fill the Windows pipe buffer; if
    # vspipe stderr is not read while ffmpeg is waiting for frames, the whole
    # pipeline deadlocks.
    ...
    vs_thread = threading.Thread(target=_drain, args=(vspipe_proc.stderr, vs_stderr_chunks), daemon=True)
    ff_thread = threading.Thread(target=_drain, args=(ffmpeg_proc.stderr, ff_stderr_chunks), daemon=True)
    vs_thread.start()
    ff_thread.start()
