...
def _build_parser() -> argparse.ArgumentParser:
    ...
    rn.add_argument("--gate-weave",       choices=["0", "1"], dest="gate_weave",
                    help="1 = subtle 1px random per-frame offset (imax_70mm); 0 = stable")
    rn.add_argument("--vignette",         type=float,
                    help="corner darkness 0..1 (polaroid)")
    rn.add_argument("--flare-len",        type=int,   dest="flare_len",
                    help="anamorphic streak length in px (anamorphic_scope)")
    rn.add_argument("--flare-thr",        type=float, dest="flare_thr",
                    help="anamorphic highlight threshold 0..1 (anamorphic_scope)")
    rn.add_argument("--teal-shadow",      type=float, dest="teal_shadow",
                    help="teal-cool shadow tint 0..1 (anamorphic_scope)")
    # vs-mlrt ML presets (Phase 18.B-ML)
    rn.add_argument("--model",            type=str,
                    help="ML model variant (vsmlrt_realesrgan_2x / vsmlrt_rife_60fps / dpir_denoise)")
    rn.add_argument("--tile",             type=int,
                    help="ML inference tile size in px (vsmlrt_realesrgan_2x, dpir_denoise)")
    rn.add_argument("--tile-pad",         type=int, dest="tile_pad",
                    help="ML inference tile padding (vsmlrt_realesrgan_2x)")
    rn.add_argument("--backend",          choices=["auto", "trt", "ort_dml", "ort_cpu"],
                    help="vsmlrt backend; auto = TRT->DirectML->CPU (all ML presets)")
    rn.add_argument("--fps-mul",          type=float, dest="fps_mul",
                    help="frame rate multiplier (vsmlrt_rife_60fps): 2.5 for 24->60, 2 for 24->48")
    rn.add_argument("--rebuild",          choices=["conservative", "balanced", "aggressive"],
                    help="Soft HD Rebuild detail amount (soft_hd_rebuild_2x)")
    rn.add_argument("--cleanup",          choices=["light", "medium", "strong"],
                    help="pre-ML cleanup strength (soft_hd_rebuild_2x)")
