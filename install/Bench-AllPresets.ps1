...
.PARAMETER MlBackend
  Drives AUDION_VS_BACKEND for ML presets (vsmlrt_*, dpir_denoise,
  soft_hd_rebuild_2x).
  "auto"    : let vsmlrt pick TRT -> ORT_DML -> ORT_CPU (default)
  "trt"     : force TensorRT (NVIDIA-only)
  "ort_dml" : force DirectML
  "ort_cpu" : force CPU
  "sweep"   : run ML presets twice -- once with "auto" (TRT on NVIDIA),
              once with "ort_cpu" (CPU baseline) -- so we can compare
              GPU acceleration vs portable fallback.
...
param(
    ...
    [ValidateSet('auto','trt','ort_dml','ort_cpu','sweep')][string]$MlBackend = 'auto',
    [string]$Palette = '',
    [string]$ReportJson = ''
)
...
foreach ($pal in $palettes) {
    ...
    Get-ChildItem -Path $palDir -Filter '*.vpy' -File | Sort-Object Name | ForEach-Object {
        $presets += [pscustomobject]@{
            ...
            Path    = $_.FullName
            IsMl    = ($_.BaseName -match '^(vsmlrt_|dpir_|soft_hd_)')
        }
    }
}
...
#     tag ("light"/"medium"/"strong"); Restoration (dehaze, derainbow, dpir)
...
#   - AUDION_VS_MODEL must NOT be set globally either: vsmlrt presets each
#     accept a different model namespace (RIFE expects rife_v4.6, ESRGAN
#     wants general-x4v3, DPIR wants drunet_color). One global value
#     breaks the other two. Per-preset overrides live in $presetOverrides.
$baseEnv = @{
    ...
    AUDION_VS_SIGMA             = '2.5'
    # ML defaults (vsmlrt_* / soft_hd_*); 'auto' lets vsmlrt pick TRT -> DML -> CPU
    AUDION_VS_BACKEND           = 'auto'
    AUDION_VS_TILE              = '384'
    AUDION_VS_TILE_PAD          = '16'
    AUDION_VS_REBUILD           = 'balanced'
    AUDION_VS_CLEANUP           = 'medium'
    # Restoration legacy
...
$presetOverrides = @{
    'vsmlrt_realesrgan_2x' = @{ AUDION_VS_MODEL = 'general-x4v3' }
    'soft_hd_rebuild_2x'   = @{ AUDION_VS_MODEL = 'general-x4v3'; AUDION_VS_REBUILD = 'balanced'; AUDION_VS_CLEANUP = 'medium' }
    'vsmlrt_rife_60fps'    = @{ AUDION_VS_MODEL = 'rife_v4.6' }
    'dpir_denoise'         = @{ AUDION_VS_MODEL = 'drunet_color'; AUDION_VS_STRENGTH = '10' }
...
function Invoke-PresetSmoke {
    param(
        ...
        [string]$UseCuda, [string]$Backend
    )
    # Wipe stale per-preset keys from previous iteration so overrides from a
    # prior preset (e.g. STRENGTH=10 set for dpir) do not leak forward.
    foreach ($k in @('AUDION_VS_STRENGTH','AUDION_VS_MODEL')) {
    ...
    Set-Item -Path 'Env:AUDION_VS_BACKEND'  -Value $Backend

    # Mode tag tells us at a glance which path the row exercised.
    # For ML presets the AUDION_VS_BACKEND value is the meaningful axis.
    $modeTag = if ($Backend -and $Backend -ne 'auto') {
        switch ($Backend) {
            'trt'     { 'TRT ' }
            'ort_dml' { 'DML ' }
            'ort_cpu' { 'oCPU' }
    ...
    } elseif ($UseCuda -eq '1') { 'CUDA' } else { 'CPU ' }
...
# Build (UseCuda, Backend) tuple list per preset
function Get-RunMatrix($preset) {
    ...
    foreach ($c in $cudaModes) {
        foreach ($b in $bks) {
            $rows += [pscustomobject]@{ UseCuda=$c; Backend=$b }
        }
    }
    return $rows
}
...
foreach ($p in $presets) {
    ...
    foreach ($row in $matrix) {
        $results += Invoke-PresetSmoke -Palette $p.Palette -Name $p.Name -VPY $p.Path `
                        -UseCuda $row.UseCuda -Backend $row.Backend
    }
}
